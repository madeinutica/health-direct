...
export function filterProviders(
  ...
  category: string,
  location: string,
  userSelectedInsurancePlans: string[],
  userAcceptsMedicaid: boolean,
  userAcceptsMedicare: boolean,
  filterInNetwork: boolean // New parameter for "In-Network" filter
): ProcessedProvider[] {
  return providers.filter(provider => {
    ...
    if (filterInNetwork) {
      ...
      // Check for Medicaid
      if (userAcceptsMedicaid && provider.acceptsMedicaid) {
        matchesInsurance = true;
      }
      // Check for Medicare
      if (userAcceptsMedicare && provider.acceptsMedicare) {
        matchesInsurance = true;
      }

      // Check for specific selected plans
      if (userSelectedInsurancePlans.length > 0 && provider.acceptsInsurance) {
        const providerAcceptsLower = provider.acceptsInsurance.map(ins => ins.toLowerCase());
        const userPlansLower = userSelectedInsurancePlans.map(ins => ins.toLowerCase());
        if (userPlansLower.some(userPlan => providerAcceptsLower.includes(userPlan))) {
          matchesInsurance = true;
        }
      }
